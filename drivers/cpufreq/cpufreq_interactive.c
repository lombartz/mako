 ...
 */
#define DEFAULT_INPUT_BOOST_FREQ 1024000
	...
	struct cpufreq_interactive_cpuinfo *pcpu)
{
	...
	expires = jiffies + usecs_to_jiffies(scale_timer_rate());
...
static void cpufreq_interactive_timer_start(int cpu)
{
	...
	unsigned long expires = jiffies + usecs_to_jiffies(scale_timer_rate());
	unsigned long flags;
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	if (new_freq < pcpu->floor_freq) {
		if (now - pcpu->floor_validate_time < scale_min_sample_time()) {
			trace_cpufreq_interactive_notyet(
				data, cpu_load, pcpu->target_freq,
				pcpu->policy->cur, new_freq);
			goto rearm;
	...
	if (!is_touching || new_freq > hispeed_freq) {
